...
When creating specs, reference `docs-ai/business-context.md` to choose the right template:

- **Frontend features** → `apps/react-app`, `apps/nextjs-app`, `apps/react-ssr`, `apps/astro-web`
- **Backend APIs** → `apps/go-clean`, `apps/go-modular`, `apps/fastapi-ai`
- **CMS features** → `apps/strapi-cms`
